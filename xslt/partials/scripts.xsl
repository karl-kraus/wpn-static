    ...
    <xsl:variable name="script_mappings" as="map(xs:string, xs:string*)">
        <xsl:map>
            ...
            <xsl:for-each select="collection('../../data/editions?select=idPb*.xml')">
                <xsl:map-entry key="tokenize(base-uri(current()),'/')[last()]" select="('wpn-header','init-micro-editor','wpn-text-view','wpn-entity','wpn-text-zoom-button', 'init-openseadragon', 'wpn-hf-height')"/>
            </xsl:for-each>
        </xsl:map>
    </xsl:variable>
