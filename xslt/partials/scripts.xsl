...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:map="http://www.w3.org/2005/xpath-functions/map" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs xsl tei map" version="3.0">
    ...
    <xsl:variable name="script_mappings" as="map(xs:string, xs:string*)">
        <xsl:map>
            ...
            <xsl:map-entry key="'visualisierungen.xml'" select="('wpn-header','wpn-toggle-text-button','wpn-scroll-button')"/>
            ...
            <xsl:for-each select="collection('../../vis?select=*.xml')">
                <xsl:map-entry key="tokenize(base-uri(current()),'/')[last()]" select="('wpn-header')"/>
            </xsl:for-each>
            <xsl:for-each select="collection('../../data/editions?select=wit-*.xml')">
                <xsl:map-entry key="tokenize(base-uri(current()),'/')[last()]" select="('wpn-header','wpn-page-view','wpn-entity', 'wpn-hf-height', 'wpn-typo-connections', 'wpn-typo-utils','init-typo-mark','wpn-first-visit-overlay')"/>
            </xsl:for-each>
