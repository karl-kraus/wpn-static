...
<xsl:stylesheet 
    ...
    <xsl:template match="tei:fw">
        <span style="z-index:1;" class="fw {replace(@change,'#','')} {replace(@rendition,'#','')} {@place}" data-hand="fw-{replace(@change,'#','')}">
            <xsl:apply-templates/>
        </span>
    </xsl:template>
    ...
    <xsl:template match="tei:note">
        <span class="note d-block text-align-left {if(@place)then(concat(@place, ' position-absolute'))else()} {replace(@change,'#','')}" data-hand="{replace(@change,'#','')}">
